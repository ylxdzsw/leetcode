...
unsafe fn solve_n_queens(n: i32) -> Vec<Vec<String>> {
    ...
    unsafe fn algorithm_x(root: *mut Node, results: &mut Vec<Vec<(usize, usize)>>, result: &mut Vec<(usize, usize)>) {
        let mut c = root;
        ...
        if c == root {
            return results.push(result.clone())
        }
        ...
        cfor!(let mut r = (*c).d; r != c; r = (*r).d; {
            ...
            result.push((*r).n);
            algorithm_x(root, results, result);
            result.pop();
    ...
    let mut results = vec![];
    algorithm_x(root, &mut results, &mut vec![]);
    ...
    results.iter().map(|result| {
        ...
        table.iter().map(|x| x.iter().collect()).collect()
    }).collect()
}

fn main() {
    assert_eq!(unsafe { solve_n_queens(4) }, [
        [".Q..",
         "...Q",
         "Q...",
         "..Q."],
